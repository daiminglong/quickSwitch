...
public class TCPInput implements Runnable
{
                ...
                while (keyIterator.hasNext() && !Thread.interrupted())
                {
                    ...
                    if (key.isValid())
                    {
                        if (key.isConnectable())
                            processConnect(key, keyIterator);//handle connect purpose
                        else if (key.isReadable())
                            processInput(key, keyIterator);//handle input process
                    }
                }
            }
        }
    ...
    private void processConnect(SelectionKey key, Iterator<SelectionKey> keyIterator)
    {
        ...
        try
        {
            ...
            {
                keyIterator.remove();
                tcb.status = TCBStatus.SYN_RECEIVED;//tcb的状态由SYN_SENT状态转换为SYN_RECEIVED状态
                ...
                outputQueue.offer(responseBuffer);//network to device queue
    ...
    private void processInput(SelectionKey key, Iterator<SelectionKey> keyIterator)
    {
        ...
        synchronized (tcb)
        {
            ...
            if (readBytes == -1)
            {
                ...
                key.interestOps(0);
                tcb.waitingForNetworkData = false;//停止等待数据，流结束
