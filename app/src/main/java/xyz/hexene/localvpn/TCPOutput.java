...
public class TCPOutput implements Runnable
{
    ...
    public TCPOutput(ConcurrentLinkedQueue<Packet> inputQueue, ConcurrentLinkedQueue<ByteBuffer> outputQueue,
                     Selector selector, LocalVPNService vpnService)//inputQueue 是device to network  outputQueue是network to device
    {
    ...
    public void run()
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                do
                {
                    currentPacket = inputQueue.poll();//并行队列中拿到包
                ...
                ByteBuffer payloadBuffer = currentPacket.backingBuffer;//payloadBuffer
                currentPacket.backingBuffer = null;
                ByteBuffer responseBuffer = ByteBufferPool.acquire();//申请一块空间
                ...
                String ipAndPort = destinationAddress.getHostAddress() + ":" +
                        destinationPort + ":" + sourcePort;// 省去源地址 因为都一致 形成一个区分流也就是不同TCP连接的key 去获取内存缓存 TCB 传输控制块
                ...
                else if (tcpHeader.isSYN())//连接的建立或是释放请求
                    processDuplicateSYN(tcb, tcpHeader, responseBuffer);
                else if (tcpHeader.isRST())//TCP出现严重错误，必须释放连接，再重新建立连接
                    closeCleanly(tcb, responseBuffer);
                else if (tcpHeader.isFIN())//TCP释放连接请求
    ...
    private void initializeConnection(String ipAndPort, InetAddress destinationAddress, int destinationPort,
            ...
            throws IOException
    {
        currentPacket.swapSourceAndDestination();//创建回复应用的包
        if (tcpHeader.isSYN())
        {
            ...
            TCB tcb = new TCB(ipAndPort, random.nextInt(Short.MAX_VALUE + 1), tcpHeader.sequenceNumber, tcpHeader.sequenceNumber + 1,
                    tcpHeader.acknowledgementNumber, outputChannel, currentPacket);//先随机生成一个VPN自己的sequenceNumber
            TCB.putTCB(ipAndPort, tcb);//建立控制块
        ...
        else
        {
            currentPacket.updateTCPBuffer(responseBuffer, (byte) TCPHeader.RST,
                    0, tcpHeader.sequenceNumber + 1, 0);//没有tcb 但又不是syn请求 则重建连接
        }
        outputQueue.offer(responseBuffer);//假装握手
    }
    ...
    {// process 握手阶段接收到来自客户端的最后一个ack包 以及释放阶段的最后一个ack包
...
//            }
            ...
            else if (tcb.status == TCBStatus.LAST_ACK)//TCP释放阶段的最后一个ack包
            {
...
//                Constant.requestBufferMap.remove(tcb.ipAndPort);//请求结束拿掉request
            ...
            try
            {
                while (payloadBuffer.hasRemaining())
                    outputChannel.write(payloadBuffer);//把取到的数据 写出去就好了
            }
            catch (IOException e)
            {
    ...
    {
        tcb.referencePacket.updateTCPBuffer(buffer, (byte) TCPHeader.RST, 0, tcb.myAcknowledgementNum + prevPayloadSize, 0);//告诉TCP 断了
